...
HN_accuracy <- function(df,
  ...
  # NNHM for the HN heterogeneity prior
  # input:
  # df: data frame in form compatible with bayesmeta
  # scale.HN: scale parameter of the HN heterogeneity prior
  ...
  # tail.prob=0.5: 50%-RLMC-adjustment of HC heterogeneity priors for tau with respect to the median RLMC induced by the HN prior
  # grid.epsilon=0.00354: grid.epsilon as suggested by Roos et al. (2015) corresponds to H(N(0,1),N(0.01,1))=0.00354
  # output:
  # list containing parameters of HN and HC, the effective median RLMC induced by the HN prior, S(tau)^HN and S(tau)^HC values
  # and the decision whether these priors are anti-conservative or conservative
  
  # checking of input values
  if(! scale.HN > 0)
    stop("The scale parameter scale.HN must be a positive real number.")
  ...
  # compute the median RLMC induced by the HN(scale.HN) prior
  mrlmc_HN <- median(
    effective_rlmc(
      df = df,
      r.tau.prior = function(n)
        rhalfnormal(n = n, scale = scale.HN),
      MM = 10 ^ 6,
      output = "sample"
    )
  )
  ...
  scaling_param = list(HN = scale.HN, HC = tau_HC_dyn)
  
  
  ####---- Epsilon-local sensitivity for HN(scale.HN) and 50%-RLMC-adjusted HC(tau_HC_dyn) ----####
